...
pub trait RequestExt: 'static {
    ...
    #[cfg(any(feature = "v2_42", feature = "dox"))]
    fn send_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<gio::InputStream, Error>) + Send + 'static>(&self, cancellable: Option<&P>, callback: Q);

    #[cfg(feature = "futures")]
    #[cfg(any(feature = "v2_42", feature = "dox"))]
    fn send_async_future(&self) -> Box_<dyn future::Future<Output = Result<gio::InputStream, Error>> + std::marker::Unpin>;
}
...
impl<O: IsA<Request>> RequestExt for O {
    ...
    fn send_async<P: IsA<gio::Cancellable>, Q: FnOnce(Result<gio::InputStream, Error>) + Send + 'static>(&self, cancellable: Option<&P>, callback: Q) {
        ...
        unsafe extern "C" fn send_async_trampoline<Q: FnOnce(Result<gio::InputStream, Error>) + Send + 'static>(_source_object: *mut gobject_sys::GObject, res: *mut gio_sys::GAsyncResult, user_data: glib_sys::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            soup_sys::soup_request_send_async(self.as_ref().to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box::into_raw(user_data) as *mut _);
        }
    }
